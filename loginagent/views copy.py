...
evil_bot_personality = [
    {"role": "system", "content": "I am a medical prescription order agent."},
    {"role": "system", "content": "I have some sign-in credentials found from a data breach."},
    {"role": "system", "content": "My goal is to see if I can buy some medicine"},
    {"role": "system", "content": "And you job is to inform the event log what you are doing"},
]
...
def get_signed_message_from_site(url): # THIS IS A FUNCTION THAT IS ACTUALLY RUN AT THE SITE WHERE WE ARE CALLING FOR THE CHALLENGE
    ...
    timestamp = datetime.datetime.utcnow().isoformat(timespec='seconds')  # e.g., "2025-02-09T15:00:00"
    # Pad or truncate to exactly 20 characters (adding 'Z' for UTC, if needed)
    timestamp = (timestamp + "Z").ljust(20)[:20]
...
def run_test(request):
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            if is_certified:
                messages = good_bot_personality
                messages.append({"role": "system", "content": "You just found the Certified bot button, let´s click it 😇"})
                found_button_response = client.chat.completions.create(
                    model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                    messages=messages,
                )
                        
                send_event('room-{}'.format(user), 'message', {'message': found_button_response.choices[0].message.content})
            else:
            ...
            # Click the submit button
                ...
                messages = evil_bot_personality
                messages.append({"role": "system", "content": "You just found the Certified bot button"})
                found_button_response = grok_client.chat.completions.create(
                    model='grok-2-1212',
                    messages=messages,
                )
                send_event('room-{}'.format(user), 'message', {'message': f'😈:{found_button_response.choices[0].message.content}'})
            ...
            if is_certified:
                ...
                messages = good_bot_personality
                messages.append({"role": "system", "content": "I just received a signed challenge from the site. Now explain how you will use the Site Public Key to check if it is safe."})
                signed_challenge_response = client.chat.completions.create(
                    model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                    messages=messages,
                )
                send_event('room-{}'.format(user), 'message', {'message': f'😇:{signed_challenge_response.choices[0].message.content}'})
                ...
                if not verify_signed_message(signed_message, settings.SITE_PUBLIC_KEY):
                    messages = good_bot_personality
                    messages.append({"role": "system", "content": "Site verification failed. Someone is trying to figure out our SECRET code!"})
                    signed_challenge_response = client.chat.completions.create(
                        model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                        messages=messages,
                    )
                ...
                messages = good_bot_personality
                messages.append({"role": "system", "content": "Site verification passed using the Site Public Key. Double check for potential abuse."})
                signed_challenge_response = client.chat.completions.create(
                    model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                    messages=messages,
                )
                ...
                if nonce in USED_NONCES or abs((datetime.datetime.fromisoformat(timestamp) - datetime.datetime.fromisoformat(current_time)).total_seconds()) > TIME_WINDOW_SECONDS:
                    messages = good_bot_personality
                    messages.append({"role": "system", "content": "Nonce reuse detected or timestamp expired. Someone is trying to figure out our SECRET code!"})
                    signed_challenge_response = client.chat.completions.create(
                        model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                        messages=messages,
                    )
                ...
                messages = good_bot_personality
                messages.append({"role": "system", "content": "Nonce reuse and timestamp freshness verified. No risk for abuse."})
                signed_challenge_response = client.chat.completions.create(
                    model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                    messages=messages,
                )
                ...
                send_event('room-{}'.format(user), 'message', {'message': f'SYSTEM: Extracted fingerprint control question from site database'})
                ...
                send_event('room-{}'.format(user), 'message', {'message': 'SYSTEM: Received fingerprint response from fine-tuned LLM Agent.'})
                ...
                send_event('room-{}'.format(user), 'message', {'message': 'SYSTEM: Encrypt LLM response with site public key and send back for verification.'})
                ...
                if simulate_site_validation(encrypted_response):
                    ...
                    send_event('room-{}'.format(user), 'message', {'message': 'SYSTEM: Certification succeeded!'})
                else:
                    certified_code = 'INVALID'
                    send_event('room-{}'.format(user), 'message', {'message': 'SYSTEM: Certification failed!'})
                ...
                messages = evil_bot_personality
                messages.append({"role": "system", "content": "There is a input field saying Certification Code, what should you do?"})
                signed_challenge_response = grok_client.chat.completions.create(
                    model='grok-2-1212',
                    messages=messages,
                )
                send_event('room-{}'.format(user), 'message', {'message': f'😈:{signed_challenge_response.choices[0].message.content}'})
           ...
           # Click the submit button
            ...
            try:
                ...
                if is_certified:
                    messages = good_bot_personality
                    messages.append({"role": "system", "content": "You just logged in successfully!"})
                    login_response = client.chat.completions.create(
                        model="ft:gpt-4o-mini-2024-07-18:personal:regentcaptcha:AyuS3lPS",
                        messages=messages,
                    )
                    send_event('room-{}'.format(user), 'message', {'message': f'😇:{login_response.choices[0].message.content}'})
                else:
                    messages = evil_bot_personality
                    messages.append({"role": "system", "content": "You just logged in successfully!"})
                    login_response = grok_client.chat.completions.create(
                        model="grok-2-1212",
                        messages=messages,
                    )
                    send_event('room-{}'.format(user), 'message', {'message': f'😈:{login_response.choices[0].message.content}'})
                ...
                if not is_certified:
                    messages = evil_bot_personality
                    messages.append({"role": "system", "content": "I was not able to log in since I am not certified!"})
                    login_response = grok_client.chat.completions.create(
                        model="grok-2-1212",
                        messages=messages,
                    )
